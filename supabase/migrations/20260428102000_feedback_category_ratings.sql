/*
  ...
  Splits shipment feedback into attitude, speed, accuracy, and price ratings.
  The existing rating column remains as the rounded overall score.
*/
...
DROP FUNCTION IF EXISTS submit_shipment_feedback(uuid, text, integer, text);
...
REVOKE ALL ON FUNCTION submit_shipment_feedback(uuid, text, integer, integer, integer, integer, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION submit_shipment_feedback(uuid, text, integer, integer, integer, integer, text) TO anon, authenticated;
