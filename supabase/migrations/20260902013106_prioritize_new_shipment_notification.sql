/*
  # Prioritize new-shipment notifications

  A newly created shipment remains a creation notification even when its
  first tracking event is entered during creation. `__status_set__` is only
  used when the first tracking status is added to an existing shipment.
*/
