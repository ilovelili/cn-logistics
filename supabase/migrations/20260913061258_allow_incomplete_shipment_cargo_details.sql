/*
  Cargo and FCL booking details may be unknown when a shipment is first
  handled. Allow either section to remain completely blank while continuing
  to validate any section or row that an operator has started entering.
*/
...
BEGIN
  ...
  IF package_count IS NOT NULL
    OR gross_weight IS NOT NULL
    OR volume IS NOT NULL THEN
    IF package_count IS NULL
      ...
      OR volume IS NULL
      OR volume <= 0 THEN
      RAISE EXCEPTION 'Started cargo details require positive PKG, Kgs and M3 values'
        USING ERRCODE = '22023';
    END IF;
  END IF;
  ...
  IF target_transport_mode = 'fcl' THEN
    FOR booking_item IN
      SELECT value FROM jsonb_array_elements(COALESCE(new_booking_details, '[]'::jsonb))
    LOOP
      IF NULLIF(trim(booking_item->>'booking_number'), '') IS NULL
        ...
        RAISE EXCEPTION 'Each started FCL booking requires a number and containers'
          USING ERRCODE = '22023';
      END IF;
