      ...
      {assignments.map((assignment) => {
        ...
        return (
          <span
            ...
            title={`${name} / ${roleLabels.join(", ")}`}
          >
            <span className="min-w-0 truncate">{name}</span>
            {roleLabels.map((roleLabel) => (
              <span
                key={roleLabel}
                className="shrink-0 rounded-full bg-cyan-100 px-1.5 py-0.5 text-[10px] font-black text-cyan-900 dark:bg-cyan-900/60 dark:text-cyan-100"
              >
                {roleLabel}
              </span>
            ))}
