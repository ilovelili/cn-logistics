...
export default function AppNotificationBell({
  ...
  const load = useCallback(() => {
    void fetchAppNotifications(profileEmail)
      .then(setItems)
      .catch(() => setItems([]));
  }, [profileEmail]);
  useEffect(load, [load]);
      ...
      {open && (
        <div className="fixed right-4 top-20 z-[150] w-[min(92vw,430px)] overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-900">
          <div className="flex items-center justify-between border-b border-gray-200 p-4 dark:border-gray-800">
            <div>
              <h3 className="font-black">{t("appNotifications.title")}</h3>
              <p className="text-xs text-gray-500">
                {t("appNotifications.unreadCount", { count: unread })}
              </p>
            </div>
            <div className="flex gap-2">
              ...
              >
                ...
                {t("appNotifications.markAllRead")}
              </button>
          ...
          <div className="flex border-b border-gray-200 dark:border-gray-800">
            {(["unread", "all"] as const).map((value) => (
              <button
                ...
                className={`flex-1 px-4 py-3 text-sm font-bold ${tab === value ? "border-b-2 border-cyan-500 text-cyan-700" : "text-gray-500"}`}
              >
                {t(
                  value === "unread"
                    ? "appNotifications.unread"
                    : "appNotifications.all",
                )}
              </button>
            ))}
          </div>
            ...
            {visible.length === 0 ? (
              ...
              visible.map((item) => (
                <button
                  ...
                  className={`block w-full p-4 text-left hover:bg-gray-50 dark:hover:bg-gray-800 ${item.read_at ? "" : "bg-cyan-50/60 dark:bg-cyan-950/20"}`}
                >
                  ...
                  <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">
                    {item.shipper_name || "-"}
                    {item.subject && item.subject !== item.shipper_name
                      ? ` / ${item.subject}`
                      : ""}
                  </p>
                  <p className="mt-1 text-xs text-gray-500">
                    {t("appNotifications.actor")}: {item.actor_email || "-"}
                  </p>
                </button>
              ))
            )}
